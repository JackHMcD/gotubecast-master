...
import (
	...
	"github.com/JackHMcD/gotubecast/mp"
)
...
func genericCmd(index int64, cmd string, paramsList []interface{}) {
	...
	case "setPlaylist":
		...
		*/
		...
		mp.StartSlave()
		mp.SendCommand("https://www.youtube.com/watch?v="curVideoId" --audio-display=no --audio-channels=stereo --audio-samplerate=48000 --audio-format=s16 --ao=pcm --ao-pcm-file=/tmp/snapcast/snapfifo --ytdl-format=bestaudio[ext=m4a]")
		/*
			postBind("nowPlaying", map[string]string{
				"videoId":      curVideoId,
